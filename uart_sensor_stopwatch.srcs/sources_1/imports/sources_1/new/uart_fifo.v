...
module uart_top (
    ...
    // 추가된 외부 연결용 포트
    output [7:0] o_rx_data, // ASCII 디코더로 갈 데이터
    output       o_rx_done, // ASCII 디코더로 갈 수신 완료 펄스
    input  [7:0] i_tx_data, // ASCII 센더에서 올 데이터
    input        i_tx_push, // ASCII 센더에서 올 송신(Push) 요청
    output       o_tx_full  // ASCII 센더로 갈 TX FIFO 가득 참 상태
);
    wire w_b_tick, w_rx_done;
    wire [7:0] w_rx_data, w_tx_fifo_pop_data;
    wire w_tx_fifo_full, w_tx_busy, w_tx_fifo_empty;
    ...
    uart_tx U_UART_TX (
        .clk(clk), .rst(rst),
        .tx_start(~w_tx_fifo_empty), .b_tick(w_b_tick),
        .tx_data(w_tx_fifo_pop_data), .tx_busy(w_tx_busy),
        .tx_done(), .uart_tx(uart_tx)
    );

    // 센더 모듈과 통신할 TX FIFO
    fifo U_FIFO_TX (
        .clk(clk), .rst(rst),
        .push(i_tx_push),       // 센더가 데이터를 밀어넣음
        .pop(~w_tx_busy),
        .push_data(i_tx_data),  // 센더의 BCD-ASCII 변환 데이터
        .pop_data(w_tx_fifo_pop_data),
        .full(w_tx_fifo_full), .empty(w_tx_fifo_empty)
    );

    uart_rx U_UART_RX (
        .clk(clk), .rst(rst), .rx(uart_rx), .b_tick(w_b_tick),
        .rx_data(w_rx_data), .rx_done(w_rx_done)
    );

    baud_tick U_BOUD_TICK (
        .clk(clk), .rst(rst), .b_tick(w_b_tick)
    );
endmodule
